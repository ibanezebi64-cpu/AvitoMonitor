...
vk.updates.on('message_new', async (context, next) => {
  ...
  if (text === 'помощь' || payload?.command === 'help') {
    await context.send({
      message: `🆘 Подробная инструкция:

1️⃣ КАК ДОБАВИТЬ ПОИСК:
— Перейдите в «Добавить категории» ➔ «Добавить по ссылке».
— Сначала напишите название для поиска (например, «Айфон 15»). Это поможет вам различать ваши поиски в списке.
— Затем перейдите на сайт Авито и настройте нужные фильтры (город, цена, состояние и т.д.).
— ❗ ВАЖНО: Обязательно выберите сортировку «По дате».
— Скопируйте ссылку из адресной строки и отправьте её боту.

2️⃣ КАК ЗАПУСТИТЬ БОТА:
— В главном меню нажмите кнопку «Запустить отслеживание».
— Когда кнопка станет красной («Остановить»), это значит, что бот начал работу.
— Бот сначала соберет текущие объявления, а затем будет присылать только новые.

3️⃣ УПРАВЛЕНИЕ СПИСКОМ:
— В меню «Мои категории» вы можете увидеть все ваши поиски, проверить ссылки или удалить ненужные.

💡 СОВЕТ: Можно добавить сразу несколько разных поисков. Бот будет присылать уведомления по всем сразу!`,
...
vk.updates.on('message_new', async (context, next) => {
  ...
  if (['старт', 'начать', '/start'].includes(text) || payload?.command === 'main' || text === 'главное меню') {
    ...
    await context.send({
      message: `Привет! Я бот для мониторинга новых объявлений на Авито.\n\nЯ буду присылать вам уведомления, как только на сайте появится новое объявление по вашим параметрам.\n\nТекущий статус: ${statusText}`,
      keyboard: MAIN_MENU_DEFAULT_KEYBOARD
    });
...
vk.updates.on('message_new', async (context, next) => {
  ...
  if (payload?.command === 'toggle_tracking') {
    ...
    const statusText = isActive ? '✅ Отслеживание успешно запущено! Я сообщу, когда появятся новые объявления.' : '⏸ Отслеживание остановлено. Новые уведомления приходить не будут.';
...
vk.updates.on('message_new', async (context, next) => {
  ...
  if (payload?.command === 'add_categories') {
    const kb = Keyboard.builder()
      .textButton({ label: '🔗 Добавить по ссылке', payload: { command: 'add_by_url' }, color: Keyboard.POSITIVE_COLOR })
      .row()
      .textButton({ label: '« Назад в меню', payload: { command: 'main' }, color: Keyboard.PRIMARY_COLOR })
      .inline(true);

    await context.send({
      message: 'Для отслеживания новых объявлений нужно добавить ссылку, для этого нажмите кнопку: "Добавить по ссылке". Далее следуйте инструкциям.',
      keyboard: kb
    });
    return;
  }
